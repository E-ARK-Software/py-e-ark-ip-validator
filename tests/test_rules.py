...
class ValidationRulesTest(unittest.TestCase):
    ...
    def test_validate_mets(self):
        ...
        rules.validate(str(files(XML).joinpath('METS-valid.xml')))
        self.assertTrue(rules.get_report().is_valid)
    ...
    def test_mets_root(self):
        result, failures, _ = _test_validation('root', 'METS-valid.xml')
    ...
    def test_mets_hdr_valid(self):
        result, failures, _ = _test_validation('hdr', 'METS-valid.xml')
        self.assertTrue(failures == 0)
        self.assertTrue(result)
    ...
    def test_mets_root_dmd(self):
        result, _, warnings = _full_validation('root', 'METS-valid.xml')
        found_csip17 = False
    ...
    def test_mets_dmd(self):
        result, failures, warnings = _test_validation('dmd', 'METS-valid.xml')
        self.assertTrue(failures == 0)
        self.assertTrue(warnings == 0)
        self.assertTrue(result)

    def test_mets_amd(self):
        result, failures, warnings = _test_validation('amd', 'METS-valid.xml')
        self.assertTrue(failures == 0)
        self.assertTrue(warnings == 0)
        self.assertTrue(result)
